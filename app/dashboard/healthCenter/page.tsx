...
export default function AmbulancePage() {
  ...
  useEffect(()=>{
    if(!user)return;
    const loadingToast = toast.loading('Cargando Centros médicos...');
    const fetchHealthCenters = async () => {
      try {
        ...
        toast.success('Centros médicos cargados.', {id: loadingToast});
      } catch (error) {
        console.error("Error fetching heath ceneter:", error);
        toast.error('Error al cargar los centros médicos.', {id: loadingToast});
      } finally {
        setLoading(false);
      }
    }
      ...
      <div className="mt-20 px-4 flex flex-col items-start ml-10 grow">
        <div className="w-10/12">
          <div className="flex justify-between items-start mb-6 w-full">
            <h1 className="text-3xl font-bold text-gray-900 mb-8">Centros médicos</h1>
            <div className="w-1/5">
              <Button
                title="Agregar"
                color="green"
